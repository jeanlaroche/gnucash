...
load_shared_qf_cb (Account *account, gpointer data)
{
    ...
    // TODO: JEAN THIS IS WHERE THE ACCOUNT NAMES ARE COLLECTED IN STORE
                           ...
                           gpointer user_data, gpointer event_data)
{
    ...
    // JEAN: Now that qfb->list_store_full has been updated, qfb->list_store also needs to be updated in
    // case we're using the regular search.
    gtk_list_store_clear(qfb->list_store);
    
    g_debug("Replicate shared_store_full\n");
    gboolean valid = gtk_tree_model_get_iter_first (GTK_TREE_MODEL(qfb->list_store_full), &iter);
    gchar *str_data = NULL;
    gint num_accounts = 0;
    while (valid)
    {
